...
# --- 50x AGGRESSIVE WEIGHTED LOSS ---
def weighted_mse_loss(pred, target):
    """
    Weights pixels with high activity (snake/food) 50x more than background.
    """
    # 1. Calculate Activity Map
    activity = torch.abs(target).sum(dim=1, keepdim=True)
    ...
    # 3. Create Weight Map: Background=1.0, Foreground=50.0 (Was 10.0)
    # This ensures the snake pixels dominate the gradient descent
    weights = 1.0 + (activity_norm * 29.0)
...
def compute_val_loss(model, dataloader, accelerator, noise_scheduler):
    ...
    with torch.no_grad():
        for history, action_map, target in dataloader:
            ...
            # Use same 50x weighted loss
            total_loss += weighted_mse_loss(pred, noise).item()
            count += 1
    model.train()
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="output/diffusion_v5")
    ...
    if os.path.exists(args.vae_path):
        try:
            ...
            print("⚠️ VAE strict load failed, trying loose...")
    ...
    print(f"--- Training Diffusion V5 (30x Weight + 30% CFG) ---")
    ...
    for epoch in range(args.epochs):
        ...
        for history, action_map, target in pbar:
            # A. Cheap Scheduled Sampling (5%)
            if np.random.random() < 0.05:
                with torch.no_grad():
                    fake_t = torch.randint(200, 400, (history.shape[0],), device=accelerator.device).long()
                    fake_noise = torch.randn_like(target)
                    fake_noisy_input = noise_scheduler.add_noise(target, fake_noise, fake_t)
                    fake_input = torch.cat([fake_noisy_input, history, action_map], dim=1)
                    fake_pred = model(fake_input, fake_t).sample
                    new_hist = history.clone()
                    new_hist[:, -LATENT_DIM:, :, :] = fake_pred
                    history = new_hist

            # B. CFG Dropout (30% - Increased)
            if np.random.random() < 0.30:
                action_map = torch.zeros_like(action_map)

            # C. Reduced History Noise (0.10 - Decreased)
            noise_scale = torch.rand(history.shape[0], 1, 1, 1, device=accelerator.device) * 0.10
            ...
            # 50x Weighted Loss
            loss = weighted_mse_loss(pred, noise)
